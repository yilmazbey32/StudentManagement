...
return  contactMessageService.save(contactMessageRequest);
    ...
    @GetMapping("/searchBetweenDates") // http://localhost:8080/contactMessages/searchBetweenDates + GET
    @PreAuthorize("hasAnyAuthority('ADMIN','MANAGER','ASSISTANT_MANAGER')")
    public ResponseEntity<List<ContactMessage>>searchBetweenDates(
            @RequestParam(value = "beginDate" )String beginDateString,
            @RequestParam(value = "EndDate")String endDateString
    ){
       List<ContactMessage> contactMessages= contactMessageService.searcByDateBetween(beginDateString,endDateString);
